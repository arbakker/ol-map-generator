...
class MapGenerator {
  ...
  static getStyle(layerConfig) {
    ...
    console.log("getStyle", layerConfig.geomType);
  ...
  static async getFeatureData(layerConfig) {
    if (
      layerConfig.layerType === "featureLayer" &&
      layerConfig.sourceType !== "service"
    ) {
      let ftCollection;
      if (layerConfig.sourceType === "url") {
        let ftCollectionString = await MapGenerator.getResourceText(
  ...
  static getFeatureUrl(config) {
    let baseUrl = config.serviceUrl.split("?")[0];
    let ftName = config.featureType.Name;
    return `${baseUrl}?version=2.0.0&service=WFS&srsname=EPSG:3857&request=GetFeature&outputFormat=application/json&typename=${ftName}&count=${config.nrFeatures}`;
  }
  ...
  generateMap() {
    ...
    config.layers.forEach((element) => {
      promises.push(MapGenerator.getFeatureData(element));
    });
    ...
    Promise.all(promises).then((values) => {
      ...
      values.forEach((result) => {
        console.log(result);
        if (
          result.config.layerType === "featureLayer" &&
          result.config.sourceType !== "service"
        ) {
          i += 1;
          layers.push(
            new VectorLayer({
          ...
          layers.push(
            new VectorLayer({
              ...
              source: new VectorSource({
                format: new GeoJSON(),
                url: (extent) =>
                  MapGenerator.getFeatureUrl(result.config) +
                  "&bbox=" +
                  extent.join(",") +
                  ",EPSG:3857",
